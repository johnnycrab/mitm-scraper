...
def arp_monitor_callback(pkt):
	if ARP in pkt and pkt[ARP].op == 1 : #who-has only
		...
		#print ''
		if (str(pkt[ARP].pdst) == '192.168.178.29') or (str(pkt[ARP].pdst) == '192.168.178.27'):
			print 'Router asking where ' + str(pkt[ARP].pdst) + ' is. This is the printing box. Send regular reply.'
			print ''
			packet = ARP()
           		packet.op = 2
           		packet.psrc = myip
		...
		elif (str(pkt[ARP].hwdst) == '00:00:00:00:00:00' and str(pkt[ARP].pdst) == gatewayip and myip != str(pkt[ARP].psrc)):
